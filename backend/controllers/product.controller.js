...
export const getFeaturedProducts = async (req, res) => {
  try {
    ...
    res.status(500).json({success: false, messagea: "Server Error", error: error.message})
  }
};
...
export const deleteProduct = async (req,res)=>{
  ...
  if(product.image){
    ...
    try {
      ...
      console.log("Erorr deleting form cloudinary ", error.message);
      res.status(401).json({success: false, error: "Server Error", error: error.message}) 
    }
  }

  await Product.findByIdAndDelete(req.params.id)
  res.json({message: "Product delete successfuly"})

  
  } catch (error) {
    console.log("Erorr in dleteProduct controller ", error.message);
    res.status(401).json({success: false, error: "Server Error", error: error.message})
  }
...
export const getRecommendedProducts = async (req,res)=>{
  try {
    ...
    console.log("Erorr in getRecommendedFeatures controller ", error.message);
    res.status(401).json({success: false, error: "Server Error", error: error.message})
  }
}
